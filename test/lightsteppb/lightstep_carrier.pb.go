...
func (*BinaryCarrier) Descriptor() ([]byte, []int) { return fileDescriptorLightstepCarrier, []int{0} }
...
func (*BasicTracerCarrier) Descriptor() ([]byte, []int) {
	return fileDescriptorLightstepCarrier, []int{1}
}
...
func (m *BinaryCarrier) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.DeprecatedTextCtx) > 0 {
		for _, b := range m.DeprecatedTextCtx {
			dAtA[i] = 0xa
			i++
			i = encodeVarintLightstepCarrier(dAtA, i, uint64(len(b)))
			i += copy(dAtA[i:], b)
		}
	}
	if m.BasicCtx != nil {
		dAtA[i] = 0x12
		i++
		i = encodeVarintLightstepCarrier(dAtA, i, uint64(m.BasicCtx.ProtoSize()))
...
func (m *BasicTracerCarrier) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.BaggageItems) > 0 {
		for k, _ := range m.BaggageItems {
			...
			i++
			v := m.BaggageItems[k]
			mapSize := 1 + len(k) + sovLightstepCarrier(uint64(len(k))) + 1 + len(v) + sovLightstepCarrier(uint64(len(v)))
			i = encodeVarintLightstepCarrier(dAtA, i, uint64(mapSize))
			dAtA[i] = 0xa
			i++
			i = encodeVarintLightstepCarrier(dAtA, i, uint64(len(k)))
			i += copy(dAtA[i:], k)
			dAtA[i] = 0x12
			i++
			i = encodeVarintLightstepCarrier(dAtA, i, uint64(len(v)))
			i += copy(dAtA[i:], v)
		}
	}
	return i, nil
...
func encodeVarintLightstepCarrier(dAtA []byte, offset int, v uint64) int {
	for v >= 1<<7 {
...
func (m *BinaryCarrier) ProtoSize() (n int) {
	...
	if len(m.DeprecatedTextCtx) > 0 {
		for _, b := range m.DeprecatedTextCtx {
			l = len(b)
			n += 1 + l + sovLightstepCarrier(uint64(l))
		}
	}
	if m.BasicCtx != nil {
...
func (m *BasicTracerCarrier) ProtoSize() (n int) {
	...
	if len(m.BaggageItems) > 0 {
		for k, v := range m.BaggageItems {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + sovLightstepCarrier(uint64(len(k))) + 1 + len(v) + sovLightstepCarrier(uint64(len(v)))
			n += mapEntrySize + 1 + sovLightstepCarrier(uint64(mapEntrySize))
		}
	}
	return n
}

func sovLightstepCarrier(x uint64) (n int) {
	for {
		n++
		x >>= 7
		if x == 0 {
...
func sozLightstepCarrier(x uint64) (n int) {
	return sovLightstepCarrier(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *BinaryCarrier) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowLightstepCarrier
			}
			if iNdEx >= l {
		...
		case 1:
			...
			if byteLen < 0 {
				return ErrInvalidLengthLightstepCarrier
			}
		...
		default:
			iNdEx = preIndex
			skippy, err := skipLightstepCarrier(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
...
func skipLightstepCarrier(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowLightstepCarrier
			}
			if iNdEx >= l {
		...
		case 2:
			...
			iNdEx += length
			if length < 0 {
				return 0, ErrInvalidLengthLightstepCarrier
			}
			return iNdEx, nil
		case 3:
			for {
				...
				next, err := skipLightstepCarrier(dAtA[start:])
				if err != nil {
					return 0, err
				}
				iNdEx = start + next
...
var (
	ErrInvalidLengthLightstepCarrier = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowLightstepCarrier   = fmt.Errorf("proto: integer overflow")
)

func init() { proto.RegisterFile("lightstep_carrier.proto", fileDescriptorLightstepCarrier) }

var fileDescriptorLightstepCarrier = []byte{
	// 350 bytes of a gzipped FileDescriptorProto
	,
}
